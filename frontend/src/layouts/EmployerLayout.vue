...
<template>
  <v-app style="background: transparent">
    <v-app-bar flat color="surface" class="jb-glass" style="box-shadow: 0 1px 0 rgba(226,232,240,0.9), 0 12px 24px -16px rgba(15,23,42,0.15);">
      ...
      <router-link to="/employer/dashboard" class="tw:no-underline tw:ml-1">
        <BrandMark size="small" />
      </router-link>
      <v-spacer />
      <v-chip v-if="mdAndUp" color="accent" variant="tonal" size="small" class="tw:mr-3">Employer</v-chip>
      <v-menu>
        <template #activator="{ props }">
          <v-btn v-bind="props" variant="text" class="tw:normal-case tw:px-2">
            <v-avatar color="secondary" size="32" :class="smAndUp ? 'tw:mr-2' : ''">
              <span class="tw:text-white tw:text-sm tw:font-semibold">
                {{ auth.displayName?.charAt(0) || 'C' }}
              </span>
            </v-avatar>
